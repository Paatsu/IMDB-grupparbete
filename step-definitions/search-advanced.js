...
module.exports = function () {
  ...
  this.Given(/^have clicked "([^"]*)" beside "([^"]*)"$/, async function (keyword, findText) {
    ...
    await driver.wait(until.elementLocated(By.css('.imdb-search-gateway__browse')));
    let findTextOnPage = await $('.imdb-search-gateway__browse');
    let findTextOnPageText = await findTextOnPage.getText();
    expect(findTextOnPageText, 'Could not find the text' + findText + ' on the page').to.include(findText);
  ...
  this.Given(/^entered the keyword "([^"]*)" in the input field beside "([^"]*)" and clicked "([^"]*)"$/, async function (keyword, findText, button) {
    ...
    await driver.wait(until.elementLocated(By.css('.searchbox')));
    let findTextOnPage = await $('.searchbox');
    let findTextOnPageText = await findTextOnPage.getText();
    expect(findTextOnPageText, 'Could not find the text' + findText + ' on the page').to.include(findText);
    await driver.wait(until.elementLocated(By.css("form.searchbox input[name='q']")));
    let inputField = await $("form.searchbox input[name='q']");
    expect(inputField, 'Could not find the input field on the page');
    await inputField.sendKeys(keyword);
    await driver.wait(until.elementLocated(By.css('input.btn')));
    let searchButton = await $('input.btn');
  ...
  this.Given(/^clicked "([^"]*)" on the "([^"]*)" "([^"]*)" page$/, async function (link, findText, keyword) {
    ...
    keywordOnPage = link;
    await driver.wait(until.elementLocated(By.css('h1.findHeader')));
    let findTextOnPage = await $('h1.findHeader');
    let findTextOnPageText = await findTextOnPage.getText();
    expect(findTextOnPageText, 'Could not find the text ' + findText + ' ' + keyword + ' on the page').to.include(findText, keyword);
    let linkOnPage = await driver.findElement(By.linkText(link));
    expect(linkOnPage, 'Could not find ' + link + ' on the page');
    await linkOnPage.click();

  });

  this.Given(/^the "([^"]*)" has loaded$/, async function (h1Text) {

    await driver.wait(until.elementLocated(By.css('h1.header')));
    let header = await $('h1.header');
    let headerText = await header.getText();
    expect(headerText, 'Could not find the header' + h1Text + ' on the page').to.include(h1Text);

  });

  this.Given(/^the "([^"]*)" corresponding to selected keyword is checked in the "([^"]*)" options \(filter\)$/, async function (keyword, refineFilter) {

    await driver.wait(until.elementLocated(By.css('div.faceter-header span.expand strong')));
    let findText = await $('div.faceter-header span.expand strong');
    let findTextText = await findText.getText();
    expect(findTextText, 'Could not find the filter option').to.include(refineFilter);
    driver.wait(until.elementLocated(By.css('.faceter-fieldset input[name="' + keywordOnPage + '"][checked="checked"]')));
    let keywordChecked = await $('.faceter-fieldset input[name="' + keywordOnPage + '"][checked="checked"]');
  ...
  this.Then(/^"([^"]*)" should be among the top (\d+) results on that page$/, async function (keyword, toplist) {
    ...
    let getSearchResult = await $('h3.lister-item-header a');
